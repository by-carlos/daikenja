...
- [`fixtures/project-card.md`](fixtures/project-card.md) -- a four-project
  ...
  never asked about -- silent in the terminal run, named as an offer when the
  same thread goes through the bot -- and the cross-check that names a
  mismatch without switching project; a cardless project reported as
  unmatchable; a handle shared by two cards deciding nothing; `project-summary` opening with the card's
...
- [`fixtures/digest-items.md`](fixtures/digest-items.md) -- a feeder's list of
  ...
  **empty** Open items section, which is the case that has to render as
  ...
  group; a list where nothing matches, which is still posted; a ledger
  carrying twenty-six open items, of which three are shown -- the shape a
  real ledger produced on the first live run; a `digest.groups` block that
  folds the two unmatched items into one paragraph each, with a channel a
  card owns listed in a group and still going to the card, a `max_chars`
