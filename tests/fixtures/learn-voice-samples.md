...
The corpus is deliberately sized between the floor and the bar -- 32 messages of
...
1. **Openers.** `Quick one --` opens 5 of the 13 chat messages that start a
   thread, and 1 of the 5 mails. It never appears in an announcement. Stable
   enough to write, and register-specific enough to be marked as such.
...
3. **Bullets past three items, prose below.** One explicit instance
   (2026-06-02 14:22) and no counter-example.
...
- **Humor.** Two instances in 32 messages, both self-deprecating, both in DM. A
...
back`, `sync up` and `touch base` appear nowhere in 32 messages -- so the
