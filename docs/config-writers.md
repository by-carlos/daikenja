...
| `writing-style.md` -- content | the user by hand, and `learn-voice` | `learn-voice` derives the file from writing samples the user supplies, backs up whatever the file held to a dated copy beside it, and writes the derived file in full. Nothing else edits it, and `learn-voice` writes nothing else apart from its dated `writing-style.watch.<YYYY-MM-DD>.md` report, always local, which no skill reads as a style file. |
...
| The project card -- `project.md` beside the ledger, per [`project-card.md`](project-card.md) § Location | `setup-project` creates it; the user writes its content by hand | Created from the template only on user approval and only when no file exists at the derived path; an existing card is never inspected or overwritten by any skill. Every skill that matches content against a project reads it. No skill writes content into it -- there is no learned or derived card, and a read skill that finds a malformed line reports it and moves on. |
